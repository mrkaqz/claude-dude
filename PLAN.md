...
| Risk | Handling |
|---|---|
| **Two BLE centrals at once** (Desktop + daemon) — the top technical unknown | Bluedroid supports multiple connections and Windows multiplexes both app sessions over one ACL link, so it should hold. **Fallback if it doesn't:** move the usage feed to Wi-Fi — the S3 has it, and a tiny HTTP POST endpoint on the device sidesteps the question entirely at the cost of Wi-Fi provisioning. Test this early, in Phase 5, before the UI is finished. |
