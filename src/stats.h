...
inline void statsOnApproval(uint32_t secondsToRespond) {
  _stats.approvals++;
  // This core's Arduino.h pulls in <algorithm>, so min() resolves to
  // std::min (template, requires matching argument types) rather than the
  // old two-argument macro upstream relied on — hence the explicit cast.
